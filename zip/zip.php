...
class aeSecureFiles {
         ...
         while ($file = @readdir ($handle)) {
            ...
            if($file != '.' && $file != '..') {
               ...
               if(is_file($path)) {
                  ...
                  // Anything returned
                  if ($subfolder_files) $all_files = array_merge($all_files, $subfolder_files);
               }
            }
         } // while ($file = @readdir ($handle))
         //
   ...
   public static function create_zip($files = array(),$destination = '',$overwrite = false) {
      ...
      if(count($valid_files)) {
         ...
         if (file_exists($destination)) {
            ...
            if (DEBUG===TRUE) $sReturn.='<p class="text-info">The zip archive contains '.$zip->numFiles.' files</p>';
         } else {
   ...
   if ($task==='doIt') {
      ...
      echo '<h2>Files compressed</h2><pre>'.print_r($files2zip,true).'</pre>';die();
