    ...
    */
    public static function php_grep($folder, $query, $filter, $links, $regex)
    {
        ...
        while (($f = readdir($fp)) !== false) {
            ...
            if ($f == "." or $f == ".." or $file_path == __FILE__ or (!$links and is_link($file_path)) or (is_file($file_path) and !fnmatch($filter, $f))) {
                continue;
            }
    ...
    switch ($task) {
        ...
        case 'doIt':
            ...
            if ($query !== "") {
                ...
                $results = ($results ? '<ol>'.$results.'</ol>' : '<p>Aucune occurence de <strong>'.$query.'</strong> n\'a été trouvée dans le dossier <strong>'.$folder.'</strong> (et sous-dossiers).</p>');
            }
