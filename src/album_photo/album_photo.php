...
<?php
   ...
   // NOTE : la variable $folder doit �tre initialis�e par le code qui appelle celui-ci
   // $folder doit �tre le nom d'un dossier se trouvant � la racine du site.   Par exemple "images/blog/album_photo".

   // Initialiser � 1 si, lorsqu'on clique sur une vignette, si l'image doit s'afficher dans une fen�tre modale.
   $useModal=1;
   ...
   // Contr�le que le dossier existe
...
if (('' != $folder) && (is_dir(JPATH_SITE . '/' . $folder))) {
    ...
    if ($count > 0) {
        // Compte le nombre de slides : divise le nombre d'images trouv� par le nombre d'images par slides et augmente de 1 s'il reste un solde
        ...
        // G�n�re le caroussel et les vignettes
        ...
        for ($i=0; $i < $slides; $i++) {
            ...
            for ($j=0; $j < $imgBySlides; $j++) {
                if (isset($arrFiles[$wPicture])) {
                    ...
                    // Contr�le s'il y a une miniature dans un sous-dossier nomm� Thumbs.  Si oui, utilise la miniature; si non, utilise l'image originale.
