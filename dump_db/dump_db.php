...
class aeSecureSession {
   ...
   function __construct($bDestroy=false) {    
      ...
      if ($bDestroy) session_destroy();
      
      if(!(isset($_SESSION['sFileName']))) self::set('sFileName','');
      if(!(isset($_SESSION['bZIP']))) self::set('bZIP',ZIP);
      
      return true;
      
   } // function __construct()   
   public static function getInstance($bDestroy=false) {
      if (self::$instance === null) self::$instance = new aeSecureSession($bDestroy);
...
class aeSecureDumpDatabase {
   ...
   public static function getInstance() {
      if (self::$instance === null) self::$instance = new aeSecureDumpDatabase();
      return self::$instance;
   }
   ...
   public function doIt() {
      ...
      if (!file_exists($dump)) { echo "Error : could not find $dump, exit."; unlink($credentialsFile); die(); }
      ...
      if (ZIP) {
	     ...
	     if($wReturn!==TRUE) { 	  
	        echo 'Error : unable to create '.$zipFile; die();		 
         } else {
            $wReturn=$zip->addFile($dump,basename($dump));     // basename() so the file will be zipped without his folder's structure
		    if($wReturn!==TRUE) { echo 'Error : unable to add '.$dump.' in the archive';$zip->close();die(); }
	     }
         $zip->close();
	     unlink($dump);
	     rename($zipFile,$dump);
   ...
   if ($task!='') {
      ...
      switch ($task) {
         ...
         case 'download' :
	        ...
	        if ($fname!=='') echo $aeDumpDB->download($fname);
			break;
   ...
   <div class="container">
      ...
      <div class="container">
         <div id="intro">
            ...
            <br/>
            <button type="button" id="btnDoIt" class="btn btn-primary">Récupère un backup de la base de données</button>
